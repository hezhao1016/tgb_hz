...
public final class CommonUtil {
    ...
    /**
     * 获取实体类中的属性
     * <br/>本方法用到了反射,其适用于所有的属性类型均为byte[]的JavaBean
     * 
    ...
    /**
     * 获取Map中的属性
     * <br/>由于Map.toString()打印出来的参数值对,是横着一排的...参数多的时候,不便于查看各参数值
     * <br/> 故此仿照commons-lang.jar中的ReflectionToStringBuilder.toString()编写了本方法
    ...
    /**
     * 获取Map中的属性
     * <br/> 该方法的参数适用于打印Map<String, byte[]>的情况
     * @return String key11=value11 \n key22=value22 \n key33=value33 \n......
     */
    ...
    /**
     * 获取Map中的属性
     * <br/> 该方法的参数适用于打印Map<String, Object>的情况
     * @return String key11=value11 \n key22=value22 \n key33=value33 \n......
     */
    ...
    /**
     * 金额元转分
     * <br/> 注意:该方法可处理贰仟万以内的金额,且若有小数位,则不限小数位的长度
     * <br/> 注意:如果你的金额达到了贰仟万以上,则不推荐使用该方法,否则计算出来的结果会令人大吃一惊
    ...
    /**
     * 金额元转分
     * <br/> 该方法会将金额中小数点后面的数值,四舍五入后只保留两位....如12.345-->12.35
     * <br/> 注意:该方法可处理贰仟万以内的金额
     * <br/> 注意:如果你的金额达到了贰仟万以上,则非常不建议使用该方法,否则计算出来的结果会令人大吃一惊
     * @param amount  金额的元进制字符串
     * @return String 金额的分进制字符串
     */
    ...
    /**
     * 金额分转元
     * <br/> 注意:如果传入的参数中含小数点,则直接原样返回
     * <br/> 该方法返回的金额字符串格式为<code>00.00</code>,其整数位有且至少有一个,小数位有且长度固定为2
    ...
    /**
     * 字节数组转为字符串
     * <br/> 该方法默认以ISO-8859-1转码
     * <br/> 若想自己指定字符集,可以使用<code>getString(byte[] data, String charset)</code>方法
     */
    ...
    /**
     * 字节数组转为字符串
     * <br/> 如果系统不支持所传入的<code>charset</code>字符集,则按照系统默认字符集进行转换
     */
     ...
     * 字符串转为字节数组
     * <br/> 该方法默认以ISO-8859-1转码
     * <br/> 若想自己指定字符集,可以使用<code>getBytes(String str, String charset)</code>方法
     */
     ...
     * <br/> 方法内部首先会过滤Map<String,String>参数中的部分键值对
     * <br/> 过滤规则:移除键名为"cert","hmac","signMsg"或者键值为null或者键值长度为零的键值对
     * <br/> 过滤结果:过滤完Map<String,String>后会产生一个字符串,其格式为[key11=value11|key22=value22|key33=value33]
     * <br/> And the calls { TradePortalUtil#getHexSign(String,String,String,boolean)}进行签名
    ...
    /**
     * 通过指定算法签名字符串
     * <br/> Calculates the algorithm digest and returns the value as a hex string
     * <br/> If system dosen't support this <code>algorithm</code>, return "" not null
     * <br/> It will Calls { TradePortalUtil#getBytes(String str, String charset)}
     * <br/> 若系统不支持<code>charset</code>字符集,则按照系统默认字符集进行转换
     * <br/> 若系统不支持<code>algorithm</code>算法,则直接返回""空字符串
     * <br/> 另外,commons-codec.jar提供的DigestUtils.md5Hex(String data)与本方法getHexSign(data, "UTF-8", "MD5", false)效果相同
    ...
    /**
     * 字符编码
     * <br/> 该方法默认会以UTF-8编码字符串
     * <br/> 若想自己指定字符集,可以使用<code>encode(String chinese, String charset)</code>方法
     */
     ...
     * 字符编码
     * <br/> 该方法通常用于对中文进行编码
     * <br/> 若系统不支持指定的编码字符集,则直接将<code>chinese</code>原样返回
     */
    ...
    /**
     * 字符解码
     * <br/> 该方法默认会以UTF-8解码字符串
     * <br/> 若想自己指定字符集,可以使用<code>decode(String chinese, String charset)</code>方法
     */
     ...
     * 字符解码
     * <br/> 该方法通常用于对中文进行解码
     * <br/> 若系统不支持指定的解码字符集,则直接将<code>chinese</code>原样返回
     */
    ...
    /**
     * 字符串右补空格
     * <br/> 该方法默认采用空格(其ASCII码为32)来右补字符
     * <br/> 若想自己指定所补字符,可以使用<code>rightPadForByte(String str, int size, int padStrByASCII)</code>方法
     */
    ...
    /**
     * 字符串右补字符
     * <br/> 若str对应的byte[]长度不小于size,则按照size截取str对应的byte[],而非原样返回str
     * <br/> 所以size参数很关键..事实上之所以这么处理,是由于支付处理系统接口文档规定了字段的最大长度
     * <br/> 若对普通字符串进行右补字符,建议org.apache.commons.lang.StringUtils.rightPad(...)
    ...
    /**
     * 字符串左补空格
     * <br/> 该方法默认采用空格(其ASCII码为32)来左补字符
     * <br/> 若想自己指定所补字符,可以使用<code>leftPadForByte(String str, int size, int padStrByASCII)</code>方法
     */
    ...
    /**
     * 获取前一天日期yyyyMMdd
     * <br/> 经测试，针对闰年02月份或跨年等情况，该代码仍有效。测试代码如下
     * <br/> calendar.set(Calendar.YEAR, 2013);
     * <br/> calendar.set(Calendar.MONTH, 0);
     * <br/> calendar.set(Calendar.DATE, 1);
     * <br/> 测试时，将其放到<code>calendar.add(Calendar.DATE, -1);</code>前面即可
    ...
    /**
     * HTML字符转义
     * <br/> 对输入参数中的敏感字符进行过滤替换,防止用户利用JavaScript等方式输入恶意代码
     * <br/> String input = <img src='http://t1.baidu.com/it/fm=0&gp=0.jpg'/>
     * <br/> HtmlUtils.htmlEscape(input);         //from spring.jar
     * <br/> StringEscapeUtils.escapeHtml(input); //from commons-lang.jar
     * <br/> 尽管Spring和Apache都提供了字符转义的方法,但Apache的StringEscapeUtils功能要更强大一些
     * <br/> StringEscapeUtils提供了对HTML,Java,JavaScript,SQL,XML等字符的转义和反转义
     * <br/> 但二者在转义HTML字符时,都不会对单引号和空格进行转义,而本方法则提供了对它们的转义
