 ...
 * 
 * 参考网址：http://blog.csdn.net/xiaoxian8023/article/details/40597835
 */  
public class ExcelUtilByPoi {  
    ...
    public ExcelUtilByPoi(){  
          
    }  
      
    public ExcelUtilByPoi(String excelPath){  
     ...
     * @return 
     */  
    public ExcelUtilByPoi RestoreSettings(){  
        ExcelUtilByPoi instance = new  ExcelUtilByPoi(this.excelPath);  
        return instance;  
    }  
    ...
    public static void main(String[] args) {  
        ExcelUtilByPoi eu = new ExcelUtilByPoi();  
...
//		eu.testMerge();
	...
	private void testRead(){
		try {  
	        ...
	        ExcelUtilByPoi eu = new ExcelUtilByPoi();  
	        eu.setExcelPath("D:\\My Documents\\桌面\\对账单.xls");  
	        ...
	        /*Sheet2
	    	 * 						考勤表								
	    		工号	姓名	本月总天数	出勤天数	缺勤天数	请假	矿工	加班	调休
	    		1	张三	22		22		0		0	0	0	0
	    		2	李四	22		24		0		0	0	2	0
	    		3	王五	22		18		0		2	0	2	0

	    	 * */
	        
	        System.out.println("\n获取后自己打印  > > > > > >");
	        for (int i = 2; i < list.size(); i++) {
				System.out.println(list.get(i).getCell(0)+" | "
						+list.get(i).getCell(1)+" | "
						+list.get(i).getCell(2)+" | "
						+list.get(i).getCell(3)+" | "
						+list.get(i).getCell(4)+" | "
						+list.get(i).getCell(5)+" | "
						+list.get(i).getCell(6)+" | "
						+list.get(i).getCell(7)+" | "
						+list.get(i).getCell(8)+" | "
					);
			}
	    } catch (IOException e) {  
	        e.printStackTrace();  
	...
	private void testMerge(){  
	    try {  
	    	ExcelUtilByPoi eu1 = new ExcelUtilByPoi();//用来读取源xls  
	    	ExcelUtilByPoi eu2 = new ExcelUtilByPoi();//用来读取目标xls，用于演示合并结果  
