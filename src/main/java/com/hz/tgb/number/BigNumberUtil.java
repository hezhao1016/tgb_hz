 ...
 * 大数工具类
 *
 * Created by hezhao on 2018/11/10 12:02
 */
public class BigNumberUtil {
    ...
    public static String subtract(String a, String b){
        ...
        //采用整数+整数，小数+小数的方式运算
        subtract(s_a_int, s_b_int, len_fraction, result);
        subtract(s_a_fraction, s_b_fraction, 1-len_fraction, result);
     ...
     * @param result	结果数组
     */
    private static void subtract(char[] a, char[] b, int start , int[] result){
        // 计算结果集合，a的位数>=b的位数
        int i = 0, j=0;
        for (; i < a.length && j< b.length; i++,j++) {
            result[Math.abs(i + start)] +=((int) (a[i] - '0') - (int) (b[j] - '0'));
